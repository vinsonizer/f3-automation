...
services.fetch = function (url, opts, oauthservice) {
  var result = ''
  if (oauthservice) result = oauthservice.fetch(url, opts)
  else result = UrlFetchApp.fetch(url, opts).getContentText()
  return result
}
...
services.webExec = function (serviceArgs, url, opts, callback) {
  services.createOauthService(serviceArgs, function (err, service) {
    if (err) callback(err)
    else if (!service.hasAccess()) callback(new Error('Access Exception: ' + service.authorize()), service)
    else {
      var result
      try {
...
services.showSideBar = function (content) {
  var html = HtmlService.createHtmlOutput(content).setTitle('F3 Nation').setWidth(300)
